...
-- ---------- Moderation (Pi App Studio Community Guidelines compliance) ----------
-- User-submitted reports on posts, comments, videos, chat messages, or other users.
-- Reviewed by an app owner/admin outside this MVP (e.g. via direct DB/admin panel);
-- the important part for Mainnet review is that reporting + blocking exist and work.
CREATE TABLE IF NOT EXISTS reports (
  id TEXT PRIMARY KEY,
  reporter_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  target_type TEXT NOT NULL,          -- 'post' | 'comment' | 'video' | 'message' | 'user'
  target_id TEXT NOT NULL,
  reason TEXT NOT NULL,               -- 'spam' | 'harassment' | 'scam_or_fraud' | 'illegal' | 'nudity' | 'other'
  details TEXT,
  status TEXT NOT NULL DEFAULT 'open', -- 'open' | 'reviewed' | 'dismissed' | 'actioned'
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
);

-- One-directional block: blocker no longer sees blocked's posts/videos and
-- blocked can no longer message blocker. Required so users can protect
-- themselves without waiting on manual moderation.
