...
// GET /api/posts — feed, newest first (posts from users you've blocked are hidden)
router.get('/', auth, (req, res) => {
  const blocked = blockedIdsFor(req.user.id);
  const rows = db.prepare('SELECT * FROM posts ORDER BY created_at DESC LIMIT 150')
    .all()
    .filter((r) => !blocked.has(r.user_id));
  res.json({ posts: rows.slice(0, 100).map((r) => serializePost(r, req.user.id)) });
});
